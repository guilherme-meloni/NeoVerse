...
import { Terminal } from './Terminal.js'; // NOVO IMPORT
...
async function init() {
  ...
  cityManager = new CityManager(universe, windowManager, objectManager);
  
  // ===== INIT TERMINAL =====
  terminal = new Terminal(universe, cityManager);

  // Expose quality switcher
...
function setupUI() {
  ...
  // Hotkey: T para abrir terminal
  document.addEventListener('keydown', (e) => {
      // Só ativa se não estiver digitando em outro input e não estiver no terminal
  ...
  // Listen para spawn de objetos via terminal
  window.addEventListener('terminal-spawn-object', async (e) => {
      const { type, position } = e.detail;
      await objectManager.addObject(type, position);
  });
}
